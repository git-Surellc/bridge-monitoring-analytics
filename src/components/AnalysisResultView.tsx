          ...
          <div className="bg-gradient-to-r from-indigo-50 to-blue-50 p-6 rounded-xl border border-indigo-100 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              <div className="bg-white/60 p-4 rounded-lg">
                ...
                <div className="text-2xl font-mono font-bold text-gray-900">{correlationResult.correlation.toFixed(3)}</div>
              </div>
              <div className="bg-white/60 p-4 rounded-lg">
                ...
                <div className={`text-xl font-mono font-bold ${correlationResult.isSignificant ? 'text-green-600' : 'text-gray-400'}`}>
                  {correlationResult.pValue.toFixed(3)}
                </div>
                  ...
                  {quality && (
                    <div className="grid grid-cols-4 gap-2 text-center text-xs">
                      <div className="bg-gray-50 p-2 rounded">
                        ...
                        <div className="font-mono font-semibold">{quality.mean.toFixed(3)}</div>
                      </div>
                      <div className="bg-gray-50 p-2 rounded">
                        <div className="text-gray-400 mb-1">CV</div>
                        <div className="font-mono font-semibold">{quality.cv.toFixed(3)}</div>
                      </div>
                      <div className="bg-gray-50 p-2 rounded">
                        ...
                        <div className={`${quality.missingRate > 5 ? 'text-red-600' : 'text-gray-900'} font-mono font-semibold`}>
                          {Number(quality.missingRate).toFixed(3)}%
                        </div>
                      </div>
                      <div className="bg-gray-50 p-2 rounded">
                  ...
                  {trend && (
                    <div className="grid grid-cols-2 gap-2 text-xs pt-2 border-t border-gray-100">
                      <div className="flex justify-between items-center px-1">
                        ...
                        <span className="font-mono font-medium text-gray-700">{trend.slope.toFixed(3)}</span>
                      </div>
                      <div className="flex justify-between items-center px-1">
                        <span className="text-gray-500">拟合度(R²)</span>
                        <span className="font-mono font-medium text-gray-700">{trend.rSquared.toFixed(3)}</span>
                      </div>
                    </div>
                  )}
                  ...
                  {deformation && (
                    <div className="space-y-2 pt-2 border-t border-gray-100">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-mono font-bold">{deformation.rangeValue.toFixed(3)}</span>
                      </div>
                      {deformation.periodicFeatures.mainPeriods.length > 0 && (
                        <div className="text-xs text-gray-500 space-y-1">
                          ...
                          <div className="text-gray-400 pl-1">
                             幅值: {deformation.periodicFeatures.amplitudes.map(a => a.toFixed(3)).join(', ')}
                          </div>
                        </div>
                      )}
                    </div>
                  ...
                  {acceleration && (
                    <div className="space-y-2 pt-2 border-t border-gray-100">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-mono font-bold">{acceleration.pga.toFixed(3)}</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        ...
                        <span className={`font-mono font-bold ${acceleration.isFreqAbnormal ? 'text-red-600' : 'text-green-600'}`}>
                          {acceleration.naturalFreq.toFixed(3)}
                        </span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-gray-500">PSD峰值</span>
                        <span className="font-mono text-gray-600">{acceleration.psdValue.toFixed(3)}</span>
                      </div>
